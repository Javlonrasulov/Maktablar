      ...
      {open ? (
        <motion.div className="fixed inset-0 z-[70]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.aside
            ...
            className={cn(
              'glass-strong absolute safe-bottom',
              isBottom && 'inset-x-0 bottom-0 max-h-[85vh] rounded-t-[22px] p-5',
              side === 'left' && 'inset-y-0 left-0 w-[min(88vw,320px)] rounded-r-[22px] p-5',
              side === 'right' && 'inset-y-0 right-0 w-[min(88vw,360px)] rounded-l-[22px] p-5',
            )}
          >
